-- Migration: video_transcode_jobs queue + HLS columns
-- ---------------------------------------------------------------------------
-- Required for server-side HLS adaptive-bitrate transcoding of reels and
-- product videos. The `transcode-reel` edge function inserts a row here, and
-- the self-hosted FFmpeg worker (scripts/transcode-worker) drains it.
--
-- This file is safe to re-run (uses IF NOT EXISTS / ADD COLUMN IF NOT EXISTS).
-- Run it in the Supabase SQL editor, or via the CLI:
--   supabase db execute --file supabase/schema/transcode-jobs.sql
-- ---------------------------------------------------------------------------
