...
serve(async (req) => {
  ...
  try {
    ...
    if (!redisEnabled || !redisUrl || !redisToken) {
      ...
      console.info(
        `[cached-product-search] source=database reason=${!redisEnabled ? "cache_disabled" : "redis_not_configured"} query="${queryText}" tag="${tag}" offset=${offset} limit=${limit}`,
      );
      return new Response(
        JSON.stringify({
          products,
    ...
    try {
      ...
      if (typeof cachedValue === "string" && cachedValue.trim()) {
        console.info(
          `[cached-product-search] source=redis cache_hit=true query="${queryText}" tag="${tag}" offset=${offset} limit=${limit}`,
        );
        return new Response(
          JSON.stringify({
    ...
    const products = await queryProducts();
    console.info(
      `[cached-product-search] source=database cache_hit=false query="${queryText}" tag="${tag}" offset=${offset} limit=${limit}`,
    );
    ...
    console.error("cached-product-search function error:", error);
    return new Response(
      JSON.stringify({
