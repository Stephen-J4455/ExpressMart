...
export const StatusViewer = ({ navigation, route }) => {
  ...
  const isAdStory = !!currentStatus?.is_ad_story;

  const openStoryCta = async (event) => {
    event?.stopPropagation?.();

    const url = String(currentStatus?.cta_url || "").trim();
    if (!url) return;

    const destination = /^[a-z][a-z0-9+.-]*:\/\//i.test(url)
      ? url
      : `https://${url}`;
    ...
    try {
      ...
      console.error("Failed to open story CTA URL", err);
    }
  };
  ...
  return (
    <Pressable style={styles.container} onPress={handleNext}>
      {currentStatus.status_type === "text" ? (
        ...
        currentStatus.gradient_start ? (
          <LinearGradient
            colors={[
              currentStatus.gradient_start,
              currentStatus.gradient_end || currentStatus.gradient_start,
            ]}
            start={{ x: 0, y: 0 }}
            end={{ x: 1, y: 1 }}
            style={styles.media}
          ...
          <View
            style={[
              styles.media,
              { backgroundColor: currentStatus.background_color || "#FFFFFF" },
            ]}
          />
        )
      ) : (
      ...
      <SafeAreaView
        style={[
          styles.overlay,
          currentStatus.status_type === "text" && styles.overlayText,
        ]}
      >
        <View style={styles.headerRow}>
          <View style={styles.sellerRow}>
            {currentStatus.seller?.avatar ? (
              <Image
                source={{ uri: currentStatus.seller.avatar }}
                style={styles.avatar}
              />
            ) : (
        ...
        {isAdStory && currentStatus.cta_url ? (
          <Pressable style={styles.storyCtaButton} onPress={openStoryCta}>
            <Text style={styles.storyCtaText}>
              {currentStatus.cta_text || "Open"}
            </Text>
